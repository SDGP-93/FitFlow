...
# Input height
input_height = float(input("Enter height (cm): "))

# Input age
input_age = int(input("Enter age: "))

# Convert input weight, height, and age to a 2D array
input_data = np.array([[input_weight, input_height, input_age]])

# Predict BMR based on gender
if gender.lower() == 'male':
    predicted_bmr = loaded_model_male_bmr.predict(input_data)
elif gender.lower() == 'female':
    predicted_bmr = loaded_model_female_bmr.predict(input_data)
else:
